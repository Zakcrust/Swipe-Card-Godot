...
func _ready():
	list_files_in_directory("res://SwipeCard/img")
	get_files_in_dir_after_import("res://SwipeCard/img")
	fileReader = File.new()
	fileReader.open("res://SwipeCard/img/img_data.txt", fileReader.READ)
	var index = 1
	var handler = 1
...
func _setImg(name):
	var img_name = str("res://SwipeCard/img/",name,".png")
	img = load(img_name)
